...
## Division of labour (from ROADMAP Phase D)

This repo is worked by two agents. **Codex owns engineering** — routing, the tab
framework, CSS and layout, the sidebar, the homepage shell, build and perf,
filters, audit scripts, accessibility plumbing. **Claude owns chemistry
content** — the text of every question and its distractors, `why`,
`misconception`, mission definitions and hints, theory prose, worked solutions,
references.

So: do not write or reword question text, `why`, `misconception`, mission copy,
theory blocks or worked solutions. Adding a *field* is fine (ship the type, the
renderer, and an empty value — never placeholder prose that reads as finished).
Flag content gaps in the summary instead of filling them.

And never renumber a question id, in any refactor. Progress is keyed on it.

## Commits

Never add `Co-Authored-By` trailers or any other Codex/Anthropic attribution
