...
// then computes:
//   - tier distribution per module/topic and Gold/Platinum coverage gaps
//   - near-duplicate MC questions (Jaccard similarity of normalized text)
//   - thin/trivial candidates (very short `why`)
// Writes nothing back — this is read-only, for a human to triage.
...
const scratch = mkdtempSync(join(tmpdir(), 'audit-corpus-'));
...
const ID_PREFIX = Object.fromEntries(Object.entries(QUIZ_BANKS).map(([mod, arr]) => [arr[0].id.slice(0, 3), mod]));
